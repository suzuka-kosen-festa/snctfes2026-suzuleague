...
返事の内容によっては engine の変更が要るため、他のどれよりも早く投げる。

個人情報の件（[#19](https://github.com/suzuka-kosen-festa/snctfes2026-suzuleague/issues/19)）は
伏字化とprivate化の依頼提出をもってクローズした。ただし
**git履歴には元の記載が残っている**ため、private化が実際に反映されたかは別途確認する。
...
1. **A のIssueを投げる**（#20・#6・#21）。所要は合計20分程度
2. 返事が来たら反映する。#20 で問題数を人数に合わせる判断が出た場合のみ
   engine の変更が発生する（それ以外は設定ファイルの書き換えで済む）
3. Scratch側の実装が上がってきたら**結合テスト**。ここが最大の未知数で、
