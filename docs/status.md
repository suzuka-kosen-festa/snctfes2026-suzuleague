...
- **cloud サーバをデプロイ済み**（2026-07-23）。接続先は
  `wss://suzuleague-cloud.onrender.com`（Singaporeリージョン・`MAX_CLIENTS=300`）。
  **150接続まで全員に配信が届くことを実測で確認**（遅延約120ms）。
...
| 優先度 | Issue | 内容 | 状態 |
|---|---|---|---|
| **P0** | [#10](https://github.com/suzuka-kosen-festa/snctfes2026-suzuleague/issues/10) | cloud サーバのセルフホスト | **デプロイ済み**・スピンダウン挙動の確認のみ残 |
