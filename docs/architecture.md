...
| **1部屋あたり128クライアントの上限**（2026-07-22 実測） | セルフホストのcloudサーバに切り替えて上限を引き上げる方針。詳細は下記 |
...
Scratch側は URL に `?cloud_host=wss://...` を付けるだけで切り替わるので、
Python側・Scratch側ともに実装変更は不要。
...
**運用上の注意**: 15分間まったく通信がないとスピンダウンし、復帰に約1分かかる。
...
1. **cloudサーバのセルフホスト**: ホスト先の確保と `cloud_host` の共有
2. **観客スマホ参加**: 観客がQRコード→TurboWarpのURLを開いて同じ問題に回答。
   回答の集計・ランキング
3. **Firestore連携**: ランキングの永続記録が必要になった場合のみ導入
4. **本番問題の投入**: 企画書内リンクのアンケート集計スプレッドシートから
   `questions.py` の問題セットを差し替える
5. **本番ルームIDへの切替**: Scratch担当のリミックス完了後、
