...
flowchart LR
    ...
    cloud[("セルフホスト cloud サーバ<br>wss://suzuleague-cloud.onrender.com<br>クラウド変数 = 共有黒板")]
    subgraph stage["ステージ（プロジェクター）"]
        scratch["Scratchプロジェクト<br>(TurboWarpで実行)<br>問題表示・バルーン演出・回答UI"]
    end
    subgraph audience["観客のスマホ（QRコードから開く）"]
        aud["観客用画面<br>/suzuleague.html<br>P2S_* を読むだけ・端末内で自己採点"]
    end
    dash <-->|"P2S_*: 進行状態"| cloud
    cloud <-->|"S2P_*: 回答・ACK"| scratch
    cloud ==>|"P2S_* を全員に同時配信"| aud
    firestore[("Firestore<br>（必要なら導入）<br>ランキング記録")]
    dash -.-> firestore
```
...
- **ステージ画面も観客のスマホも、同じ黒板を見ている**。司会PCが状態を書き換えると
  全員に同時に届くので、進行は自動的に揃う → [進行の同期](#進行の同期)

## 進行の同期

**ステージ上の出演者と観客が、同じタイミングで次の問題に進む。**
これはクラウド変数の性質から自動的に成立しており、同期のための特別な仕組みは要らない。

### なぜ揃うのか

クラウド変数は「**同じ部屋に繋いでいる全クライアントへのブロードキャスト**」として動く。
司会PCが `P2S_QID`（問題の番号）を書き換えると、サーバがその瞬間に接続中の全員へ配信する。
ステージ画面と観客のスマホは、**別々に問い合わせているのではなく同じ配信を受け取っている**。

したがって、以下を満たせば観客の画面はステージと同じ進行になる。

1. 同じ **room ID**（`SUZULEAGUE_PROJECT_ID`）に繋ぐ
2. 同じ **cloud サーバ**（`?cloud_host=wss://suzuleague-cloud.onrender.com`）に繋ぐ
3. `P2S_SEQ` の変化を検知して他の変数を読み直す（ステージ画面と同じ作り）
...
**150クライアントに配信しても、最も速い端末と最も遅い端末の差は数ミリ秒**（150接続時で
中央値113.4ms・最大117.4ms）。人間が知覚できる差ではないので、
「ステージだけ先に問題が変わる」ということは起きない。

### 途中から参加した観客も追いつく

cloud サーバは**新規接続時に現在の変数値をまとめて送る**。
イベントの途中でQRコードを読んだ観客も、接続した瞬間に「今3チーム目の4問目」という
状態を受け取れるので、その問題から参加できる。待たせる必要はない。

### 観客側は「読むだけ」

観客用の画面は `P2S_*` を**読むだけ**で、送信するのは接続時の handshake のみ。
...
ページは cloud サーバの `public/` から配信される（**サーバを増やさずに済む**）。

### 回答は端末の中だけで採点する

観客の回答は**サーバへ送らない**。正解発表時に届く `P2S_CORRECT` と
自分の回答の差を、その端末の中でバルーンから引く。

この方式を選んだ理由:

- **通信が増えない**。観客が何人来ても受信のみなので、
  300人が一斉に回答を送る場合の負荷（最大9万メッセージ）が発生しない
- **企画書の要件を満たす**。司会台本は「挑戦者と同じ問題に参加できます」であり、
  観客全体の集計やランキングには言及していない
- 実装が単純で、8月末までに確実に仕上がる

観客全体の平均をステージに出したくなった場合は、`S2P_*` とは別の変数を新設して
集計する形になる。その際は事前に負荷検証が必要。

### バルーンはチームごとにリセットされる

観客のバルーンは、ステージのチームが変わるたびに100へ戻る。
挑戦者と同じ条件（1チーム5問・100スタート）で比べられるようにするため。
20問通しで採点すると、ほぼ確実に途中で0になってしまう。
...
最小構成でPython⇔Scratch間の通信を成立させることを優先し、
UIは後から差し替えられるようロジックと分離した上でCLIを実装した。

**本番もこのCLIで進行する。** Web UIは作らない。操作するのは司会補助1人で、
必要なのは「次へ」を押すことがほぼすべてだからGUIの利点が薄く、
本番当日にブラウザという不確定要素を増やしたくない。
...
不調時のバックアップとして、司会PC上でcloud-serverを動かして
...
Python側の実装は出そろっている。残るのは**他者の成果物との結合**と
**実地での確認**で、コードを書く作業ではない。

| 項目 | 状態 |
|---|---|
| cloudサーバのセルフホスト | ✅ 完了・稼働中（`wss://suzuleague-cloud.onrender.com`） |
| 本番問題の投入 | ✅ Python側は20問投入済み。Scratch側の表示リストへの取り込み待ち |
| 観客スマホ参加 | ✅ 実装・デプロイ済み。**実機での表示確認が残る** |
| **本番ルームIDへの切替** | ⬜ Scratch担当のプロジェクトID待ち。`SUZULEAGUE_PROJECT_ID` に設定するだけ |
| **Scratch側との結合テスト** | ⬜ **未着手。最大の未知数**（下記） |
| Firestore連携 | 保留。ランキングの永続記録が必要になった場合のみ導入 |

### 結合テストがなぜ最大の未知数か

現在の動作確認はすべて `sim_scratch.py`——**このリポジトリの作者が
自分の仕様通りに書いたシミュレータ**——を相手にしている。これが通るのは当然で、
Scratch側が [protocol.md](./protocol.md) の通りに実装されている保証にはならない。

繋いだときに出やすいのは次のあたり。

- 変数名の綴り違い（クラウド変数は名前が違っても**エラーにならず、ただ届かない**）
- 数値の型。Scratch側の値が文字列として送られてくる場合がある
- 連続した更新の取りこぼし。`P2S_SEQ` の変化で読み直す作りになっているか
- タイミング。Python側が書いた瞬間にScratch側が読みに行っていないケース

いずれも**エラーを出さずに「動いていないように見える」**種類の不具合なので、
実際に繋ぐまで発見できない。Scratch側の実装が上がり次第、最優先で結合する。
